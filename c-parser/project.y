...
%token int_const char_const float_const id string enumeration_const storage_const type_const qual_const struct_const enum_const DEFINE IFNDEF
...
%token point_const param_const ELSE INCLUDE
%left '+' '-'
%left '*' '/'
%nonassoc THEN
%nonassoc ELSE
...
%%
program_unit				: IFNDEF program_unit
							| DEFINE primary_exp program_unit
							| INCLUDE program_unit
							| translation_unit
							;
...
{
	extern int yylineno;
	printf("Parsing Failed in line number: %d\n%s\n", yylineno, msg);
	errors++;
}
